...
const PokemonItem = ({ data }: PokemonItemProps) => {
  ...
  return (
    <div className="mx-5 lg:mx-0">
      <div
        className="p-2 border rounded-lg shadow-xl cursor-pointer bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-green-500 hover:to-yellow-400 group min-h-[430px]"
        title={data.name}
      >
          ...
          <div className="h-full group-hover:scale-125 ease-in-out duration-200 flex items-center">
            <div className="lazyload-wrapper flex">
              <NavLink to={`/${ROUTES.DETAIL}/${data.name}`}>
                <LazyLoadImage src={`${IMAGE_URL}/${data.id}.png`} alt={`${data.name}`} />
              </NavLink>
            </div>
          </div>
        </div>
